...
def sanity_check():
    ...
    if params.particle_type == "ICOSPHERE" and params.ico_subdivisions > 6:
...
def generate_particle(location = Vector((0,0,0)), rotation = Euler((0,0,0)), scale = 1, name = "None"):
    ...
    match params.particle_type:
        ...
        case "CYLINDER":
            depth = min(params.cylinder_ratio, 1)
            diameter = min(1 / params.cylinder_ratio, 1)
            bpy.ops.mesh.primitive_cylinder_add(vertices = params.cylinder_faces, radius = 0.5 * diameter, depth = depth)
        case "STL":
...
def main():
    if (not params.supress_warnings):
        sanity_check()
